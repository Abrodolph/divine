...
/**
 * Field + column definitions for the config-driven modules (RecordManager).
...
export const ITEMS = {
  fields: [
    ...
    { key: 'size_hint', label: 'Size hint (when typed)', type: 'text', placeholder: 'e.g. 100MM' },
    { key: 'spec', label: 'Specification', type: 'textarea', full: true, placeholder: 'Class / grade / make' },
    { key: 'active', label: 'Active (shown when raising requests)', type: 'checkbox', default: true },
  ],
  columns: [
    { key: 'name', label: 'Item' },
    { key: 'category', label: 'Category' },
    { key: 'unit', label: 'Unit' },
    { key: 'sizes', label: 'Sizes', type: 'tags' },
    { key: 'last_price', label: 'Last price', value: (r) => (r.last_price != null ? `₹${Number(r.last_price).toLocaleString('en-IN')}` : '—') },
...
export const VENDORS = {
  fields: [
    { key: 'name', label: 'Vendor / firm name', type: 'text', required: true },
    ...
    { key: 'categories', label: 'Supplies', type: 'tags', full: true, placeholder: 'Pipes, Valves, Consumables' },
  ...
  columns: [
    ...
    { key: 'categories', label: 'Supplies', type: 'tags' },
    { key: 'active', label: 'Active', type: 'bool' },
  ],
};
